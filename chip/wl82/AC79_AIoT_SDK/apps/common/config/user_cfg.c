...
static int bt_mac_is_valid_unicast(const u8 mac[6])
{
    u8 i;
    u8 all_zero = 1;
    u8 all_ones = 1;
...
const u8 *bt_get_mac_addr(void)
{
    ...
    if (jieli_chip_mac_get_wifi != NULL && jieli_chip_mac_get_wifi(mac_addr) == 0 &&
        bt_mac_is_valid_unicast(mac_addr)) {
        return mac_addr;
    }
#endif

    if (syscfg_read(CFG_BT_MAC_ADDR, mac_addr, 6) == 6 && bt_mac_is_valid_unicast(mac_addr)) {
        return mac_addr;
    }

#if defined(CONFIG_BT_MAC_USE_WIFI_FALLBACK) && defined(CONFIG_WIFI_ENABLE) && !defined(CONFIG_RF_TEST_ENABLE)
    wifi_get_mac(mac_addr);
    if (bt_mac_is_valid_unicast(mac_addr)) {
        return mac_addr;
    } else
#endif
    {
...
void cfg_file_parse(void)
{
    ...
    if (rf_fcc_adj_res_read("edr", &bt_power)) {
        log_info("\n ---  [RF_FCC_ADJ]bt_power %d ---- \n", bt_power);
    } else {
        log_info("\n ---  [RF_FCC_ADJ]bt_power NULL ---- \n");
    }

    if (rf_fcc_adj_res_read("ble", &ble_power)) {
        log_info("\n ---  [RF_FCC_ADJ]ble_power %d ---- \n", ble_power);
    } else {
        log_info("\n ---  [RF_FCC_ADJ]ble_power NULL ---- \n");
    }
#endif
