...
extern int jieli_chip_mac_get_wifi(u8 mac[6]) __attribute__((weak));
...
static int mac_is_valid_unicast(const char mac[6])
{
    return !(mac[0] & 0x01) && bytecmp((u8 *)mac, 0x00, 6) && bytecmp((u8 *)mac, 0xFF, 6);
}

static int get_tuya_wifi_mac(char mac[6])
{
    if (jieli_chip_mac_get_wifi == NULL || jieli_chip_mac_get_wifi((u8 *)mac) != 0 ||
        !mac_is_valid_unicast(mac)) {
        return -1;
    }
    return 0;
}

static int apply_wifi_mac(char *mac)
{
    int ret = wifi_set_mac(mac);
...
#endif
    int wifi_ret = apply_wifi_mac(mac);
    if (ret >= 0 && wifi_ret < 0) {
        ret = wifi_ret;
    }
...
static int oauth_get_macaddr(char *resp_buf, void *priv)
{
    ...
    int ret = set_flash_wifi_mac((char *)mac_data);
    if (ret < 0) {
        printf("set_flash_wifi_mac err, 0x%x\n", ret);
    }
...
{
    static u8 first = 1;
    if (get_tuya_wifi_mac(macaddr) == 0) {
        int ret = apply_wifi_mac(macaddr);
        if (ret < 0) {
            printf("apply Tuya WiFi MAC failed, 0x%x\n", ret);
            return ret;
        }
        memcpy(s_mac_addr, macaddr, 6);
        server_assign_macaddr_ok = 1;
        first = 0;
        printf("wifi use Tuya stable mac[%02x:%02x:%02x:%02x:%02x:%02x]\r\n",
