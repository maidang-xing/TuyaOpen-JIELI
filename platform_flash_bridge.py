...
# Extra context for the generic "downloader unavailable" error, set by
# _default_flash_command when it rejects an incomplete tools directory.
_flash_bridge_detail: str | None = None
...
def _default_flash_command(image: Path) -> tuple[list[str], Path] | None:
    ...
    missing = [path.name for path in (executable, config, uboot, cfg_tool) if not path.is_file()]
    if missing:
        # isd_config.ini is generated by 'make pre_build' next to the app
        # image and describes the flash layout of that very build (single vs
        # dual bank).  Reusing a config from another checkout would flash a
        # wrong layout, so require a completed build instead of falling back.
        global _flash_bridge_detail
        _flash_bridge_detail = (
            f"Jieli downloader incomplete in {tools_dir} (missing: {', '.join(missing)}); "
            "run a full tos.py build first - make pre_build regenerates isd_config.ini "
            "for this build's flash layout"
        )
        return None
    ...
    *,
    ...
    if command_text:
        ...
        if default is None:
            ...
            if _flash_bridge_detail:
                message = f"{message} ({_flash_bridge_detail})"
