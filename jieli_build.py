...
def create_staging_tree(
    ...
    extra_sources += (
        ...
        "    ../../../../../tuyaos_adapter/src/driver/tkl_gpio.c \\\n"
        "    ../../../../../tuyaos_adapter/src/driver/tkl_timer.c \\\n"
        "    ../../../../../tuyaos_adapter/src/driver/tkl_watchdog.c \\\n"
    ...
    content += f"    -I{tuyaopen_root}/tools/porting/adapter/gpio \\\n"
    content += f"    -I{tuyaopen_root}/tools/porting/adapter/watchdog \\\n"
    ...
    if full_stack:
        # Dual-bank (双备份) OTA: the define drives three vendor consumers —
        # apps/common/update/net_update.c stages into the backup bank,
        # cpu/wl82/tools/isd_config_rule.c emits the BR22_TWS_DB flash
        # layout, and download.c adds -update_files normal so isd_download
        # produces the db_update_files_data.bin OTA package.
        content += (
            "DEFINES += -DCONFIG_NET_ENABLE=1 -DCONFIG_BT_ENABLE=1 -DCONFIG_TWS_ENABLE "
            "-DCONFIG_BTCTRLER_TASK_DEL_ENABLE -DCONFIG_LMP_CONN_SUSPEND_ENABLE "
            "-DCONFIG_LMP_REFRESH_ENCRYPTION_KEY_ENABLE -DCONFIG_DOUBLE_BANK_ENABLE=1\n"
        )
...
def sanitize_host_path(path: str) -> str:
    """Drop POSIX-shell entries so the Windows vendor make uses cmd.exe.

    GNU make for Windows searches PATH for sh.exe and switches its SHELL to
    it when found (e.g. tos.py launched from Git Bash).  The vendor recipes
    then eat the backslashes out of tool paths (``C:\\JL\\pi32`` becomes
    ``C:JLpi32``) and every command exits 127.  Only ``.../usr/bin`` style
    entries are removed; ``Git\\cmd`` stays so git itself keeps working.
    """
    keep = []
    for entry in path.split(os.pathsep):
        if not entry:
            continue
        if "usr/bin" in entry.replace("\\", "/").lower():
            continue
        keep.append(entry)
    return os.pathsep.join(keep)


# Window [0x3A0000, 0x3F5000): above the second app bank and the vendor
# reserved areas the layout tool places below it (measured FLASH INFO with
# a ~1.06 MiB app: bank boundary 0x1CD000, VM 0x397000-0x39F000, BTIF up to
# 0x3A0000), and below the vendor USER area at 0x3F5000.
TUYA_RESERVED_START = 0x3A0000
TUYA_RESERVED_SIZE = 0x55000


def configure_tuya_reserved_area(ini_path: Path) -> bool:
    """Reserve a flash window for TuyaOpen data in the generated isd_config.

    With CONFIG_DOUBLE_BANK_ENABLE the vendor layout gives the second app
    bank the whole upper half of the flash and protects it as code: the old
    TuyaOpen partitions at 0x300000 were erased by the flasher and became
    unwritable.  A reserved area with OPT=1 survives flashing, is excluded
    from the code protection, and caps the second bank below it (still ~1.7
    MiB of OTA capacity).  The ini is regenerated by make pre_build on every
    build, so this must re-run after each build; returns True when patched.
    """
    content = ini_path.read_text(encoding="utf-8", errors="replace")
    if "TUYA_ADR" in content:
        return False
    block = (
        f"TUYA_ADR=0x{TUYA_RESERVED_START:X}; [TuyaOpen data, dual-bank OTA keeps hands off]\n"
        f"TUYA_LEN=0x{TUYA_RESERVED_SIZE:X};\n"
        "TUYA_OPT=1;\n"
    )
    # Keep the block inside [RESERVED_CONFIG] ahead of the PRCT (protect)
    # lines so the protection range calculation sees the reserved window.
    marker = "PRCT_ADR=0;"
    if marker not in content:
        raise BuildError(f"Jieli isd_config.ini has no PRCT block: {ini_path}")
    patched = content.replace(marker, block + marker, 1)
...
def find_ug_artifact(tools_dir: Path) -> Path:
    # isd_download writes the dual-bank OTA payload next to the app image
    # when invoked with -update_files normal (see cpu/wl82/tools/download.c).
    for name in ("db_update_files_data.bin", "update-ota.ufw"):
        candidate = tools_dir / name
        if candidate.is_file() and candidate.stat().st_size > 0:
            return candidate
    raise BuildError(
        f"Jieli postbuild produced no dual-bank OTA package in {tools_dir}; "
        "expected db_update_files_data.bin or update-ota.ufw"
    )


def tuya_ug_name(params: Mapping[str, str]) -> str:
    project = params.get("CONFIG_PROJECT_NAME", "jieli_uart_hello")
    version = params.get("CONFIG_PROJECT_VERSION", "1.0.0")
    return f"{project}_UG_{version}.bin"


def build_ota_package_command(tools_dir: Path, app_image: Path) -> list[str]:
    """Package-only isd_download invocation for the dual-bank OTA image.

    Mirrors the CONFIG_SFC branch of cpu/wl82/tools/download.c minus the
    device-facing flags: without -wait/-reboot isd_download packages the
    files, reports 'Device Offline' and exits cleanly in about a second.
    """
    return [
        str(tools_dir / "isd_download.exe"),
        str(tools_dir / "isd_config.ini"),
        "-gen2",
        "-tonorflash",
        "-dev",
        "wl82",
        ...
        "-div1",
        "-uboot",
        str(tools_dir / "uboot.boot"),
        "-app",
        str(app_image),
        str(tools_dir / "cfg_tool.bin"),
        "-res",
        "cfg",
        "-extend-bin",
        "-update_files",
